...
Visual-only AgileX Scout model shared by ROS visualizers and the vehicle TF
publisher. This branch targets ROS Melodic on Ubuntu Bionic and builds with
`catkin`.
...
The onboard compose launch lives in `agilex_onboard_autostart` and loads
`scout_visual.urdf` from this package.
